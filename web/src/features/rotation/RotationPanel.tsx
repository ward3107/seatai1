    ...
    <div className="bg-gray-50 rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-gray-700">{t('rotation.title')}</span>
          {rotationPlan && rotationPlan.periods.length > 0 && (
      ...
      {open && (
        <div className="p-4 pt-0 space-y-3">
          <p className="text-[11px] text-gray-500 leading-relaxed">{t('rotation.description')}</p>

          {/* Period count stepper */}
          <div className="flex items-center justify-between gap-2">
            <span className="text-xs font-medium text-gray-600">{t('rotation.periods_label')}</span>
            <div className="flex items-center gap-2">
              <button
              ...
              <span className="w-6 text-center text-sm font-semibold text-gray-700" aria-live="polite">
                {count}
              </span>
              <button
          ...
          {errorText && (
            <div role="alert" className="p-2 bg-red-50 border border-red-200 rounded-lg text-red-600 text-xs">
              {errorText}
            </div>
          )}
          ...
          {rotationPlan && rotationPlan.periods.length > 0 && !generating && (
            <div className="space-y-2">
                ...
                {rotationPlan.periods.map((p) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[10px] text-gray-400">{getDisplayScorePct(p.result)}%</span>
                    </button>
                  );
                })}
              </div>
              ...
              <button
                ...
                className="w-full py-1.5 px-3 text-xs text-gray-500 hover:text-red-600 flex items-center justify-center gap-1.5 transition-colors"
              >
                <Trash2 size={13} />
                {t('rotation.clear')}
              </button>
          ...
          {students.length < 2 && (
            <p className="text-[10px] text-gray-400">{t('rotation.need_students')}</p>
          )}
        </div>
      )}
    </div>
